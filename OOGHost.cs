...
namespace Magic
{
    public class OOGHost
    {
        ...
        public OOGHost(string server) : this(new GameServer(server), new SocketFactory())
        {
        ...
        {
            Send(gamePacket, GamePacket.GetOnePacketIdentifier(gamePacket));
        }
        public void Send(GamePacket gamePacket, PacketIdentifier packetId)
        {
            lock(sendStream)
            {
                Processor.ProcessClientPacket(gamePacket, packetId, sendStream);
